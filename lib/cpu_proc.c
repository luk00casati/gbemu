...
static void proc_cb(cpu_context *ctx) {
    ...
    switch(bit){
        ...
        cpu_set_flags(ctx, res == 0, 0, 0, setC);
        }return;
...
static void proc_cp(cpu_context *ctx) {
    ...
    ((int)ctx->regs.a) - ((int)ctx->fetched_data & 0x0F) < 0,
    n < 0);
}
...
static void proc_add(cpu_context *ctx) {
    ...
    bool is16bit = is_16_bit(ctx->cur_inst->reg_1);
    if (is16bit){
        emu_cycles(1);
    }
    ...
    int z = (val & 0xFF) == 0;
    int h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) 
    + (ctx->fetched_data & 0xF) >= 0x10;

    int c = (int)(cpu_read_reg(ctx->cur_inst->reg_1) & 0xFF) 
    + (int)(ctx->fetched_data & 0xFF) >= 0x100;

    if (is16bit){
        int z = -1;
        int h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xFFF) 
        + (ctx->fetched_data & 0xFFF) >= 0x1000;
    ...
    if (ctx->cur_inst->reg_1 == RT_SP){
        z = 0;
        h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) 
        + (ctx->fetched_data & 0xF) >= 0x10;
